/*******************************************************************************
 * Size: 48 px
 * Bpp: 1
 * Opts: 
 ******************************************************************************/

#ifdef LV_LVGL_H_INCLUDE_SIMPLE
#include "lvgl.h"
#else
#include "lvgl/lvgl.h"
#endif

#ifndef CUEBAND_48
#define CUEBAND_48 1
#endif

#if CUEBAND_48

/*-----------------
 *    BITMAPS
 *----------------*/

/*Store the image of the glyphs*/
static LV_ATTRIBUTE_LARGE_CONST const uint8_t glyph_bitmap[] = {
    /* U+F04B "" */
    0x0, 0x0, 0x0, 0x0, 0x0, 0xf, 0x0, 0x0,
    0x0, 0x0, 0x7, 0xf0, 0x0, 0x0, 0x0, 0x3,
    0xff, 0x0, 0x0, 0x0, 0x0, 0xff, 0xf0, 0x0,
    0x0, 0x0, 0x3f, 0xfe, 0x0, 0x0, 0x0, 0xf,
    0xff, 0xe0, 0x0, 0x0, 0x3, 0xff, 0xfe, 0x0,
    0x0, 0x0, 0xff, 0xff, 0xc0, 0x0, 0x0, 0x3f,
    0xff, 0xfc, 0x0, 0x0, 0xf, 0xff, 0xff, 0xc0,
    0x0, 0x3, 0xff, 0xff, 0xf8, 0x0, 0x0, 0xff,
    0xff, 0xff, 0x80, 0x0, 0x3f, 0xff, 0xff, 0xf8,
    0x0, 0xf, 0xff, 0xff, 0xff, 0x80, 0x3, 0xff,
    0xff, 0xff, 0xf0, 0x0, 0xff, 0xff, 0xff, 0xff,
    0x0, 0x3f, 0xff, 0xff, 0xff, 0xf0, 0xf, 0xff,
    0xff, 0xff, 0xfe, 0x3, 0xff, 0xff, 0xff, 0xff,
    0xe0, 0xff, 0xff, 0xff, 0xff, 0xfe, 0x3f, 0xff,
    0xff, 0xff, 0xff, 0xcf, 0xff, 0xff, 0xff, 0xff,
    0xfb, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xfe, 0xff, 0xff, 0xff, 0xff, 0xff,
    0x3f, 0xff, 0xff, 0xff, 0xff, 0x8f, 0xff, 0xff,
    0xff, 0xff, 0x83, 0xff, 0xff, 0xff, 0xff, 0x80,
    0xff, 0xff, 0xff, 0xff, 0xc0, 0x3f, 0xff, 0xff,
    0xff, 0xc0, 0xf, 0xff, 0xff, 0xff, 0xc0, 0x3,
    0xff, 0xff, 0xff, 0xe0, 0x0, 0xff, 0xff, 0xff,
    0xe0, 0x0, 0x3f, 0xff, 0xff, 0xe0, 0x0, 0xf,
    0xff, 0xff, 0xe0, 0x0, 0x3, 0xff, 0xff, 0xf0,
    0x0, 0x0, 0xff, 0xff, 0xf0, 0x0, 0x0, 0x3f,
    0xff, 0xf0, 0x0, 0x0, 0xf, 0xff, 0xf8, 0x0,
    0x0, 0x3, 0xff, 0xf8, 0x0, 0x0, 0x0, 0xff,
    0xf8, 0x0, 0x0, 0x0, 0x3f, 0xfc, 0x0, 0x0,
    0x0, 0xf, 0xfc, 0x0, 0x0, 0x0, 0x1, 0xfc,
    0x0, 0x0, 0x0, 0x0, 0x1e, 0x0, 0x0, 0x0,
    0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,

    /* U+F04C "" */
    0x3f, 0xff, 0x0, 0x3f, 0xff, 0x1f, 0xff, 0xe0,
    0x1f, 0xff, 0xef, 0xff, 0xfc, 0xf, 0xff, 0xff,
    0xff, 0xff, 0x3, 0xff, 0xff, 0xff, 0xff, 0xc0,
    0xff, 0xff, 0xff, 0xff, 0xf0, 0x3f, 0xff, 0xff,
    0xff, 0xfc, 0xf, 0xff, 0xff, 0xff, 0xff, 0x3,
    0xff, 0xff, 0xff, 0xff, 0xc0, 0xff, 0xff, 0xff,
    0xff, 0xf0, 0x3f, 0xff, 0xff, 0xff, 0xfc, 0xf,
    0xff, 0xff, 0xff, 0xff, 0x3, 0xff, 0xff, 0xff,
    0xff, 0xc0, 0xff, 0xff, 0xff, 0xff, 0xf0, 0x3f,
    0xff, 0xff, 0xff, 0xfc, 0xf, 0xff, 0xff, 0xff,
    0xff, 0x3, 0xff, 0xff, 0xff, 0xff, 0xc0, 0xff,
    0xff, 0xff, 0xff, 0xf0, 0x3f, 0xff, 0xff, 0xff,
    0xfc, 0xf, 0xff, 0xff, 0xff, 0xff, 0x3, 0xff,
    0xff, 0xff, 0xff, 0xc0, 0xff, 0xff, 0xff, 0xff,
    0xf0, 0x3f, 0xff, 0xff, 0xff, 0xfc, 0xf, 0xff,
    0xff, 0xff, 0xff, 0x3, 0xff, 0xff, 0xff, 0xff,
    0xc0, 0xff, 0xff, 0xff, 0xff, 0xf0, 0x3f, 0xff,
    0xff, 0xff, 0xfc, 0xf, 0xff, 0xff, 0xff, 0xff,
    0x3, 0xff, 0xff, 0xff, 0xff, 0xc0, 0xff, 0xff,
    0xff, 0xff, 0xf0, 0x3f, 0xff, 0xff, 0xff, 0xfc,
    0xf, 0xff, 0xff, 0xff, 0xff, 0x3, 0xff, 0xff,
    0xff, 0xff, 0xc0, 0xff, 0xff, 0xff, 0xff, 0xf0,
    0x3f, 0xff, 0xff, 0xff, 0xfc, 0xf, 0xff, 0xff,
    0xff, 0xff, 0x3, 0xff, 0xff, 0xff, 0xff, 0xc0,
    0xff, 0xff, 0xff, 0xff, 0xf0, 0x3f, 0xff, 0xff,
    0xff, 0xfc, 0xf, 0xff, 0xff, 0xff, 0xff, 0x3,
    0xff, 0xff, 0x7f, 0xff, 0x80, 0x7f, 0xff, 0x8f,
    0xff, 0xc0, 0xf, 0xff, 0xc0,

    /* U+F073 "" */
    0x0, 0x7e, 0x0, 0x1f, 0x80, 0x0, 0x1f, 0x80,
    0x7, 0xe0, 0x0, 0x7, 0xe0, 0x1, 0xf8, 0x0,
    0x1, 0xf8, 0x0, 0x7e, 0x0, 0x0, 0x7e, 0x0,
    0x1f, 0x80, 0x0, 0x1f, 0x80, 0x7, 0xe0, 0x3,
    0xff, 0xff, 0xff, 0xff, 0xf1, 0xff, 0xff, 0xff,
    0xff, 0xfe, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xfc, 0x0,
    0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
    0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xf, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xfc, 0xf,
    0xc0, 0xfc, 0xf, 0xff, 0x3, 0xf0, 0x3f, 0x3,
    0xff, 0xc0, 0xfc, 0xf, 0xc0, 0xff, 0xf0, 0x3f,
    0x3, 0xf0, 0x3f, 0xfc, 0xf, 0xc0, 0xfc, 0xf,
    0xff, 0x3, 0xf0, 0x3f, 0x3, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xfc, 0xf, 0xc0,
    0xfc, 0xf, 0xff, 0x3, 0xf0, 0x3f, 0x3, 0xff,
    0xc0, 0xfc, 0xf, 0xc0, 0xff, 0xf0, 0x3f, 0x3,
    0xf0, 0x3f, 0xfc, 0xf, 0xc0, 0xfc, 0xf, 0xff,
    0x3, 0xf0, 0x3f, 0x3, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xf7, 0xff, 0xff, 0xff, 0xff, 0xf8, 0xff,
    0xff, 0xff, 0xff, 0xfc,

    /* U+F272 "" */
    0x0, 0x7e, 0x0, 0x1f, 0x80, 0x0, 0x1f, 0x80,
    0x7, 0xe0, 0x0, 0x7, 0xe0, 0x1, 0xf8, 0x0,
    0x1, 0xf8, 0x0, 0x7e, 0x0, 0x0, 0x7e, 0x0,
    0x1f, 0x80, 0x0, 0x1f, 0x80, 0x7, 0xe0, 0x3,
    0xff, 0xff, 0xff, 0xff, 0xf1, 0xff, 0xff, 0xff,
    0xff, 0xfe, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xfc, 0x0,
    0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
    0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xf, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xfe, 0x0,
    0x0, 0x1f, 0xff, 0xff, 0x80, 0x0, 0x7, 0xff,
    0xff, 0xe0, 0x0, 0x1, 0xff, 0xff, 0xf8, 0x0,
    0x0, 0x7f, 0xff, 0xfe, 0x0, 0x0, 0x1f, 0xff,
    0xff, 0x80, 0x0, 0x7, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xf7, 0xff, 0xff, 0xff, 0xff, 0xf8, 0xff,
    0xff, 0xff, 0xff, 0xfc,

    /* U+F274 "" */
    0x0, 0x7e, 0x0, 0x1f, 0x80, 0x0, 0x1f, 0x80,
    0x7, 0xe0, 0x0, 0x7, 0xe0, 0x1, 0xf8, 0x0,
    0x1, 0xf8, 0x0, 0x7e, 0x0, 0x0, 0x7e, 0x0,
    0x1f, 0x80, 0x0, 0x1f, 0x80, 0x7, 0xe0, 0x3,
    0xff, 0xff, 0xff, 0xff, 0xf1, 0xff, 0xff, 0xff,
    0xff, 0xfe, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xfc, 0x0,
    0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
    0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xf, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xf3, 0xff, 0xff, 0xff, 0xff, 0xf8, 0x7f,
    0xff, 0xff, 0xff, 0xfc, 0xf, 0xff, 0xff, 0xff,
    0xfe, 0x1, 0xff, 0xff, 0xff, 0xff, 0x0, 0xff,
    0xff, 0xff, 0xff, 0x80, 0x7f, 0xff, 0xff, 0x1f,
    0xc0, 0x3f, 0xff, 0xff, 0x83, 0xe0, 0x1f, 0xff,
    0xff, 0xc0, 0xf0, 0xf, 0xff, 0xff, 0xe0, 0x18,
    0x7, 0xff, 0xff, 0xfc, 0x0, 0x3, 0xff, 0xff,
    0xff, 0x80, 0x1, 0xff, 0xff, 0xff, 0xf0, 0x0,
    0xff, 0xff, 0xff, 0xfe, 0x0, 0x7f, 0xff, 0xff,
    0xff, 0xc0, 0x3f, 0xff, 0xff, 0xff, 0xf8, 0x1f,
    0xff, 0xff, 0xff, 0xff, 0xf, 0xff, 0xff, 0xff,
    0xff, 0xe7, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xf7, 0xff, 0xff, 0xff, 0xff, 0xf8, 0xff,
    0xff, 0xff, 0xff, 0xfc,

    /* U+F410 "" */
    0x3f, 0xff, 0xff, 0xff, 0xff, 0xfc, 0x7f, 0xff,
    0xff, 0xff, 0xff, 0xfe, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0x3f, 0xfc, 0xff, 0xff, 0xff, 0xfe, 0x1f, 0xf8,
    0x7f, 0xff, 0xff, 0xfc, 0xf, 0xf0, 0x3f, 0xff,
    0xff, 0xf8, 0x7, 0xe0, 0x1f, 0xff, 0xff, 0xf0,
    0x3, 0xc0, 0xf, 0xff, 0xff, 0xf0, 0x1, 0x80,
    0xf, 0xff, 0xff, 0xf8, 0x0, 0x0, 0x1f, 0xff,
    0xff, 0xfc, 0x0, 0x0, 0x3f, 0xff, 0xff, 0xfe,
    0x0, 0x0, 0x7f, 0xff, 0xff, 0xff, 0x0, 0x0,
    0xff, 0xff, 0xff, 0xff, 0x80, 0x1, 0xff, 0xff,
    0xff, 0xff, 0xc0, 0x3, 0xff, 0xff, 0xff, 0xff,
    0xc0, 0x3, 0xff, 0xff, 0xff, 0xff, 0x80, 0x1,
    0xff, 0xff, 0xff, 0xff, 0x0, 0x0, 0xff, 0xff,
    0xff, 0xfe, 0x0, 0x0, 0x7f, 0xff, 0xff, 0xfc,
    0x0, 0x0, 0x3f, 0xff, 0xff, 0xf8, 0x0, 0x0,
    0x1f, 0xff, 0xff, 0xf0, 0x1, 0x80, 0xf, 0xff,
    0xff, 0xf0, 0x3, 0xc0, 0xf, 0xff, 0xff, 0xf8,
    0x7, 0xe0, 0x1f, 0xff, 0xff, 0xfc, 0xf, 0xf0,
    0x3f, 0xff, 0xff, 0xfe, 0x1f, 0xf8, 0x7f, 0xff,
    0xff, 0xff, 0x3f, 0xfc, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0x7f, 0xff, 0xff, 0xff, 0xff, 0xfe, 0x3f, 0xff,
    0xff, 0xff, 0xff, 0xfc,

    /* U+F83E "" */
    0x0, 0x3, 0xff, 0xfe, 0x0, 0x0, 0x0, 0x0,
    0x0, 0x7f, 0xff, 0xf0, 0x0, 0x0, 0x0, 0x0,
    0xf, 0xff, 0xff, 0x80, 0x0, 0x0, 0x0, 0x0,
    0xff, 0xff, 0xf8, 0x0, 0x0, 0x0, 0x0, 0xf,
    0xff, 0xff, 0x80, 0x0, 0x0, 0x0, 0x0, 0xff,
    0xff, 0xf8, 0x0, 0x0, 0x0, 0x0, 0xf, 0xc0,
    0x1f, 0x80, 0x0, 0x0, 0x0, 0x0, 0xfc, 0x1,
    0xf8, 0x0, 0x0, 0x0, 0x0, 0xf, 0xc0, 0x1f,
    0x80, 0x0, 0x0, 0x0, 0x0, 0xfc, 0x1, 0xf8,
    0x0, 0x0, 0x0, 0x0, 0xf, 0xc0, 0x1f, 0x80,
    0x0, 0x0, 0x0, 0x0, 0xfc, 0x1, 0xf8, 0x0,
    0x0, 0x0, 0x0, 0xf, 0xc0, 0x1f, 0x80, 0x0,
    0x0, 0x0, 0x0, 0xfc, 0x1, 0xf8, 0x0, 0x0,
    0x0, 0x0, 0xf, 0xc0, 0x1f, 0x80, 0x0, 0x0,
    0x0, 0x0, 0xfc, 0x1, 0xf8, 0x0, 0x0, 0x0,
    0x0, 0xf, 0xc0, 0x1f, 0x80, 0x0, 0x0, 0x0,
    0x0, 0xfc, 0x1, 0xf8, 0x0, 0x0, 0x0, 0xff,
    0xff, 0xc0, 0x1f, 0x80, 0xf, 0xff, 0xff, 0xff,
    0xfc, 0x1, 0xf8, 0x1, 0xff, 0xff, 0xff, 0xff,
    0xc0, 0x1f, 0x80, 0x3f, 0xff, 0xff, 0xff, 0xfc,
    0x1, 0xf8, 0x3, 0xff, 0xff, 0xff, 0xff, 0x80,
    0x1f, 0x80, 0x3f, 0xff, 0xff, 0xff, 0xf0, 0x1,
    0xf8, 0x3, 0xff, 0xff, 0x0, 0x0, 0x0, 0x1f,
    0x80, 0x3f, 0x0, 0x0, 0x0, 0x0, 0x1, 0xf8,
    0x3, 0xf0, 0x0, 0x0, 0x0, 0x0, 0x1f, 0x80,
    0x3f, 0x0, 0x0, 0x0, 0x0, 0x1, 0xf8, 0x3,
    0xf0, 0x0, 0x0, 0x0, 0x0, 0x1f, 0x80, 0x3f,
    0x0, 0x0, 0x0, 0x0, 0x1, 0xf8, 0x3, 0xf0,
    0x0, 0x0, 0x0, 0x0, 0x1f, 0x80, 0x3f, 0x0,
    0x0, 0x0, 0x0, 0x1, 0xf8, 0x3, 0xf0, 0x0,
    0x0, 0x0, 0x0, 0x1f, 0x80, 0x3f, 0x0, 0x0,
    0x0, 0x0, 0x1, 0xf8, 0x3, 0xf0, 0x0, 0x0,
    0x0, 0x0, 0x1f, 0x80, 0x3f, 0x0, 0x0, 0x0,
    0x0, 0x1, 0xf8, 0x3, 0xf0, 0x0, 0x0, 0x0,
    0x0, 0x1f, 0xff, 0xff, 0x0, 0x0, 0x0, 0x0,
    0x1, 0xff, 0xff, 0xf0, 0x0, 0x0, 0x0, 0x0,
    0x1f, 0xff, 0xff, 0x0, 0x0, 0x0, 0x0, 0x1,
    0xff, 0xff, 0xf0, 0x0, 0x0, 0x0, 0x0, 0xf,
    0xff, 0xfe, 0x0, 0x0, 0x0, 0x0, 0x0, 0x7f,
    0xff, 0xc0, 0x0
};


/*---------------------
 *  GLYPH DESCRIPTION
 *--------------------*/

static const lv_font_fmt_txt_glyph_dsc_t glyph_dsc[] = {
    {.bitmap_index = 0, .adv_w = 0, .box_w = 0, .box_h = 0, .ofs_x = 0, .ofs_y = 0} /* id = 0 reserved */,
    {.bitmap_index = 0, .adv_w = 672, .box_w = 42, .box_h = 50, .ofs_x = 0, .ofs_y = -7},
    {.bitmap_index = 263, .adv_w = 672, .box_w = 42, .box_h = 42, .ofs_x = 0, .ofs_y = -3},
    {.bitmap_index = 484, .adv_w = 672, .box_w = 42, .box_h = 48, .ofs_x = 0, .ofs_y = -6},
    {.bitmap_index = 736, .adv_w = 672, .box_w = 42, .box_h = 48, .ofs_x = 0, .ofs_y = -6},
    {.bitmap_index = 988, .adv_w = 672, .box_w = 42, .box_h = 48, .ofs_x = 0, .ofs_y = -6},
    {.bitmap_index = 1240, .adv_w = 768, .box_w = 48, .box_h = 42, .ofs_x = 0, .ofs_y = -3},
    {.bitmap_index = 1492, .adv_w = 960, .box_w = 60, .box_h = 42, .ofs_x = 0, .ofs_y = -3}
};

/*---------------------
 *  CHARACTER MAPPING
 *--------------------*/

static const uint16_t unicode_list_0[] = {
    0x0, 0x1, 0x28, 0x227, 0x229, 0x3c5, 0x7f3
};

/*Collect the unicode lists and glyph_id offsets*/
static const lv_font_fmt_txt_cmap_t cmaps[] =
{
    {
        .range_start = 61515, .range_length = 2036, .glyph_id_start = 1,
        .unicode_list = unicode_list_0, .glyph_id_ofs_list = NULL, .list_length = 7, .type = LV_FONT_FMT_TXT_CMAP_SPARSE_TINY
    }
};



/*--------------------
 *  ALL CUSTOM DATA
 *--------------------*/

#if LV_VERSION_CHECK(8, 0, 0)
/*Store all the custom data of the font*/
static  lv_font_fmt_txt_glyph_cache_t cache;
static const lv_font_fmt_txt_dsc_t font_dsc = {
#else
static lv_font_fmt_txt_dsc_t font_dsc = {
#endif
    .glyph_bitmap = glyph_bitmap,
    .glyph_dsc = glyph_dsc,
    .cmaps = cmaps,
    .kern_dsc = NULL,
    .kern_scale = 0,
    .cmap_num = 1,
    .bpp = 1,
    .kern_classes = 0,
    .bitmap_format = 0,
#if LV_VERSION_CHECK(8, 0, 0)
    .cache = &cache
#endif
};


/*-----------------
 *  PUBLIC FONT
 *----------------*/

/*Initialize a public general font descriptor*/
#if LV_VERSION_CHECK(8, 0, 0)
const lv_font_t cueband_48 = {
#else
lv_font_t cueband_48 = {
#endif
    .get_glyph_dsc = lv_font_get_glyph_dsc_fmt_txt,    /*Function pointer to get glyph's data*/
    .get_glyph_bitmap = lv_font_get_bitmap_fmt_txt,    /*Function pointer to get glyph's bitmap*/
    .line_height = 50,          /*The maximum line height required by the font*/
    .base_line = 7,             /*Baseline measured from the bottom of the line*/
#if !(LVGL_VERSION_MAJOR == 6 && LVGL_VERSION_MINOR == 0)
    .subpx = LV_FONT_SUBPX_NONE,
#endif
#if LV_VERSION_CHECK(7, 4, 0) || LVGL_VERSION_MAJOR >= 8
    .underline_position = -3,
    .underline_thickness = 2,
#endif
    .dsc = &font_dsc           /*The custom font data. Will be accessed by `get_glyph_bitmap/dsc` */
};



#endif /*#if CUEBAND_48*/

